...
func TestCache_Record_Concurrency(t *testing.T) {
	c := stats.NewCache()
	accountID := "acc_test_race"
	...
	got := c.Get(accountID)
	if got.CallCount != int64(workers) {
		t.Errorf("expected %d calls, got %d", workers, got.CallCount)
	}
}